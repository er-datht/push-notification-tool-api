...
`express`. It replaces the Rails `ecs-api` path for push-notification test runs. Today it is a
scaffold with `GET /health`; the first real endpoint is `POST /api/test_notification/auto_app_pushes`.
...
- `prisma/schema.prisma` is the source of truth. Change a model → `yarn db:migrate --name x` →
  commit the schema and the new `prisma/migrations/*` folder together. Never alter tables in
  TablePro on a shared environment.
...
Vitest + supertest, integration style: real app, real MySQL. `NODE_ENV=test` (set by Vitest)
silences the logger and Prisma's query log. `afterAll(() => prisma.$disconnect())` in any test
file that touches the database, so the worker exits. Add unit tests for `service.ts` logic as it
appears; keep router tests to the HTTP surface (status, headers, envelope).
